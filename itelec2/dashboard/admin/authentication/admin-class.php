...
{   
    ...
    public function sendResetLink($email) {
        ...
        if ($user) {
            ...
            // Store token in the database with expiration (consider using a new table for tokens)
    
            // Send email with the reset link
            $reset_link = "http://itelec2.com/path/to/reset_password.php?token=" . $token;
            // Use mail function or PHPMailer to send the email
        }
    }
    ...
    public function storePasswordResetToken($email, $token) {
        $stmt = $this->runQuery("UPDATE user SET reset_token = :token, token_expiration = DATE_ADD(NOW(), INTERVAL 1 HOUR) WHERE email = :email");
    ...
    public function isEmailExists($email) {
        ...
        return $result ? true : false; // Return true if email exists, otherwise false
    }
